...
SCurveInTimeConfig = 'configuration/SCurveAnalysis_FPix_WBC156.xml'
SCurveAbsConfig    = 'configuration/SCurveAnalysis_FPix.xml'
pixelIntimeVanaExe = './bin/linux/x86_64_slc5/PixelIntimeVana.exe'
...
def CheckTrimDefaultFiles(run):
    ...
    trimfile1=open('%s/Run_%d/Run_%d/TrimDefault.dat'%(rundir,runfolder(run),run))
    trimfile2=open('%s/Run_%d/Run_%d/TrimDefault_abs.dat'%(rundir,runfolder(run),run))
    n1 = len([line for line in trimfile1])
    n2 = len([line for line in trimfile2])
    if ( n1 != n2 ):
        cmd = "cat %s/Run_%d/Run_%d/TrimDefault.dat | awk '{ print $2 }' > intime.txt"%(rundir,runfolder(run),run)
        os.system(cmd)
        cmd = "cat %s/Run_%d/Run_%d/TrimDefault_abs.dat | awk '{ print $2 }' > abs.txt"%(rundir,runfolder(run),run)
        os.system(cmd)
